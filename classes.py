...
def start_music(filename):
    pygame.mixer.music.load(f"{DATA_DIR}/{SOUNDS_BACKGROUNDS_DIR}/{filename}")
...
def load_sound(name, volume=1.0):
    sound = pygame.mixer.Sound(f"{DATA_DIR}/{SOUNDS_BACKGROUNDS_DIR}/{name}")
...
class Player(AnimatedSprite):
    ...
    def start_killing_process(self, filename, sound=load_sound("silent.wav")):
...
class BulletMob(pygame.sprite.Sprite):
    ...
    def shoot(self):
        if self.bullet_directory == "up":
            x, y = self.rect.centerx - BULLET_SIZE // 2, self.rect.top - BULLET_SIZE
        elif self.bullet_directory == "down":
            x, y = self.rect.centerx - BULLET_SIZE // 2, self.rect.bottom
        elif self.bullet_directory == "right":
            x, y = self.rect.right, self.rect.centery - BULLET_SIZE // 2
        else:
            x, y = self.rect.left - BULLET_SIZE, self.rect.centery - BULLET_SIZE // 2
...
class Bullet(pygame.sprite.Sprite):
    def __init__(self, left, top, directory, distance):
        ...
        self.rect = pygame.rect.Rect(left, top, BULLET_SIZE, BULLET_SIZE)
        self.mask = pygame.mask.from_surface(self.image)
...
class GameoverWindow(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.image = pygame.Surface(self.size.size)
        self.image.fill((50, 180, 255))
        self.image.blit(pygame.transform.scale(load_image(f"{SOUNDS_BACKGROUNDS_DIR}/gameover.png", color_key=-1),
                                               (self.size.width, int(self.size.height * 0.75))), (0, 0))
