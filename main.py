...
def market(screen, cursor_image):
    ...
    home_button = Button(
        ...
        rect=pygame.rect.Rect(int(WIDTH * 0.1) - 25,
                              int(HEIGHT * 0.02), 50, 50))
    buttons = []
    font = pygame.font.Font(None, 50)
    text = font.render("20", True, (0, 0, 0))
    ...
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                for i, button in enumerate(buttons):
                    if button.check_pressed(event.pos) and i + 1 not in bought_skins and q_coins >= 20:
                        q_coins -= 20
        ...
        for i, skin in enumerate(hero.frames):
            ...
            tmp = pygame.Surface((skin_width, int(dy * 0.75)))
            tmp.fill((255, 60, 0))
            buttons.append(Button(image=tmp, rect=pygame.rect.Rect(x, y + int(skin_height * 0.81), *tmp.get_size())))
            x += skin_width + dx
        ...
        for button in buttons:
            button.draw(screen)
            screen.blit(text, (button.rect.x, button.rect.y))

        text1 = font.render(str(q_coins), True, (100, 255, 100))
        screen.blit(text1, (int(WIDTH * 0.8), int(HEIGHT * 0.02)))
